...
# puts movies.first.has_genre?('Camedy')
online = TopMovies::Netflix.new(file_name)
# puts online.by_genre.comedy
# puts online.by_country.usa
# puts online.by_genre
# puts movie = online.filter(period: :new).first
# puts movie = online.filter(genre: 'Comedy', period: :classic).first
# movie.matches_all?( {genre: ['Comedy, Drama'], year: 1993...1998} )
